...
def generate_4by4(show=False):
    """Create a 4×4 grid: rows=topologies, cols=N values, shared y-axis."""

    # First pass: collect global y-range across ALL topologies and sizes
    ...
    for row, topo in enumerate(TOPOLOGIES):
        ...
        for col, N in enumerate(N_VALUES):
            ...
            ax.grid(True, which="major", linewidth=0.3, color="#b0b0b0",
                    alpha=0.6)
            ax.grid(True, which="minor", linewidth=0.15, color="#d0d0d0",
                    alpha=0.4)
